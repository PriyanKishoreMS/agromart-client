...
import { DeleteLandData, getFilteredLandServices } from "../api/usersApi";
...
const Dashboard = () => {
  ...
  const { isError, isLoading, isSuccess, data, error, totalPages } = useQuery(
    ...
    () => getFilteredLandServices(searchQuery, page, limit)
  );

  const queryClient = useQueryClient();

  const deleteLandMutation = useMutation(DeleteLandData, {
    onSuccess: () => {
  ...
  const handleDetailClick = (data, index, id) => {
    navigate(`/landleasedetail/${index + 1}`, { state: { data, id, index } });
  };
  ...
  const handleConfirmDelete = async () => {
    if (selectedItemId) {
      await deleteLandMutation.mutateAsync(selectedItemId);
      hideConfirmation();
